...
## V6.1.4 使用說明
這一版優先處理「畫面看得到但任何按鈕都沒有反應」。
即使主程式初始化失敗，頁面也會在約 1.8 秒內直接跳出錯誤視窗，不再只顯示右上角警告數量。


## V6.1.4 修正：樂譜音名解析
實際錯誤已確認為 `Fs5 / Cs5` 等音名使用 `s` 表示 sharp，
但 V6.1.3 的五線譜定位解析器只接受 `#` 或 `b`，因此正則比對回傳 null，
接著讀取 `m[3]` 時造成：
...
同時新增明確的 `INVALID_NOTE_NAME` 錯誤。
未來若匯入電子譜含不合法音名，網頁會直接顯示是哪一個音名無法解析，
而不是只顯示模糊的 null 錯誤。
